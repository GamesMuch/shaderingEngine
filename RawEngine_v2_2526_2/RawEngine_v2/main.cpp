...
double hasHitSphere(const point3& center, double radius, const rayCast& r) {
...
glm::vec4 sendRay(rayCast r, vec3 point, double size) {
    auto t = hasHitSphere(point, size, r);
    if (t > 0.0) {
        vec3 N = glm::normalize(r.at(t) - point);
        ...
        // std::cout << pos.z << std::endl;
            ...
            for (int j = 0; j < g_height; j++) {
                for (int i = 0; i < g_width; i++) {
                    ...
                    //Changes wont register.

                    //Test test


                    pixels[j * g_width +i] = sendRay(r, orb.center, orb.radius);
